...
func main() {
	...
	teamA := team.NewTeam(1, "Chicago Foxes", nil, []*player.Player{p1})
	teamA.Budget = team.Budget{Balance: 50_000_000, SalaryCap: 30_000_000, Payroll: 10_000_000}

	teamB := team.NewTeam(2, "Boston Eagles", nil, []*player.Player{p2})
	...
	fmt.Printf("\nTeams in the database: %d\n", len(dbTeams))
	for _, t := range dbTeams {
		fmt.Printf(" - ID: %d | Team: %s | Balance: %d $\n", t.ID, t.Name, t.Budget.CurrentBalance())
	}
}
